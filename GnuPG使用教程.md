GnuPG 备忘录
====
![gnupg](https://www.gnupg.org/share/logo-gnupg-light-purple-bg.png)

### 相关软件

[GnuPG](https://www.gnupg.org/download/index.html)  |  [Gpg4win](https://www.gpg4win.org/download.html)
...
1.会出现一大段文字：先是关于软件的介绍和一些版权信息，然后询问你选择哪种加密算法，我选择默认的**RSA and RSA**，因为目前来说，**RSA**算法是安全性最强的。   
...
| 空     | --list-keys              | 列出密钥                           |
| 空     | --list-sigs              | 列出密钥和签名                     |
| 空     | --check-sigs             | 列出并检查密钥签名                 |
| 空     | --fingerprint            | 列出密钥和指纹                     |
| -K     | --list-secret-keys       | 列出私钥                           |
| 空     | --gen-key                | 生成一副新的密钥对                 |
| 空     | --delete-keys            | 从公钥钥匙环里删除密钥             |
| 空     | --delete-secret-keys     | 从私钥钥匙环里删除密钥             |
| 空     | --sign-key               | 为某把密钥添加签名                 |
| 空     | --lsign-key              | 为某把密钥添加本地签名             |
| 空     | --edit-key               | 编辑某把密钥或为其添加签名         |
| 空     | --gen-revoke             | 生成一份吊销证书                   |
| 空     | --export                 | 导出密钥                           |
| 空     | --send-keys              | 把密钥导出到某个公钥服务器上       |
| 空     | --recv-keys              | 从公钥服务器上导入密钥             |
| 空     | --search-keys            | 在公钥服务器上搜寻密钥             |
| 空     | --refresh-keys           | 从公钥服务器更新所有的本地密钥     |
| 空     | --import                 | 导入/合并密钥                      |
| 空     | --card-status            | 打印智能卡状态                     |
| 空     | --card-edit              | 更改智能卡上的数据                 |
| 空     | --change-pin             | 更改智能卡的 PIN                   |
| 空     | --update-trustdb         | 更新信任度数据库                   |
| 空     | --print-md 算法 [文件]   | 使用指定的散列算法打印报文散列值   |
...
| 空      | --textmode               | 使用标准的文本模式                 |
| -o     | --output                 | 指定输出文件                       |
| -v     | --verbose                | 详细模式                           |
| -n     | --dry-run                | 不做任何改变                       |
| -i     | --interactive            | 覆盖前先询问                       |
| 空      | --openpgp                | 行为严格遵循 OpenPGP 定义          |
| 空      | --pgp2                   | 生成与 PGP 2.x 兼容的报文          |